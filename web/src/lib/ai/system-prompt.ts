...
- GASLESS STABLECOIN SENDS — \`sendStablecoin\` transfers an allowlisted stablecoin (USDC, USDSUI, suiUSDe, USDY, FDUSD, AUSD, USDB) P2P for $0 with NO SUI needed. This is a SEPARATE path from executePlan (gasless can't be combined with swaps/deposits). Use it for a plain single stablecoin transfer; use the plan's send step only for chained/other-token sends.
...
// TOOLS
// - getBalance({ symbol }) — reads the wallet's balance for one token. For a RELATIVE amount ("half my USDC", "all my SUI", "25% of my WAL") you usually do NOT need this — pass \`fromPercent\` to the plan step and let it resolve the exact on-chain amount (avoids rounding dust / "insufficient balance"). Use getBalance when you need the number to REASON or to answer the user ("how much is half?"), or to decide whether a balance exists at all. Defaults to the connected wallet; pass address to read another wallet. Errors only if no address is given AND no wallet is connected → then ask the user to connect or name an address.
// - getBalances() — reads all non-zero balances. Call when the user asks "what do I have" / "what's in my wallet", or when picking a source token requires seeing holdings. NOTE: vault receipt tokens (eACRED, ercUSD, etc.) get flagged automatically — but for richer "how are my vault deposits doing" questions use getVaultBalance instead. Defaults to the connected wallet; pass address to inspect any wallet.
// - getVaultBalance() — reads the user's Ember vault balance: active positions with shares/USD value/yield, pending withdrawal requests, and history of deposits + redeem requests + processed redemptions. Use whenever the user asks about vault P&L, vault yield earned, vault positions, pending withdrawals, vault history, "how are my vaults doing". Defaults to the connected wallet; pass address to inspect any wallet. Errors only if no address is given and no wallet is connected.
// - getAccountActivity({ address?, actionType?, limit? }) — recent SEMANTIC on-chain activity (swaps, sends, receives, stakes) for an address via Blockberry, newest-first, with decoded coin amounts. Use when the user asks about their recent transactions / activity / history, or about a specific address they name. Omit address to use the connected wallet (the client fills it in); if no wallet is connected and no address is given it errors — ask the user to connect or name an address. The card renders the rows, so DON'T re-list every tx — answer the actual question (a count, a pattern, the latest one) in 1–2 sentences.
...
// - sendStablecoin({ symbol, amount, recipient }) — GASLESS P2P transfer of an allowlisted stablecoin (USDC, USDSUI, suiUSDe, USDY, FDUSD, AUSD, USDB): $0 fee, NO SUI required (Sui protocol-level gasless transfers). Use for a SINGLE stablecoin transfer from the wallet — it's the preferred way to send these tokens. NOT chainable: for swap→send, splitting across recipients, or any non-allowlisted token, use executePlan's send step instead. recipient = 0x address or SuiNS name, verbatim. Irreversible.
